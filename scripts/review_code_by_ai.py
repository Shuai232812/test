    ...
    for file_data in parsed_diff:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for f in concurrent.futures.as_completed(future):
                try:
                    new_comments = f.result()
                    print(f"New comments from hunk: {new_comments}")
                    if new_comments:
...
def analyze_hunk(file_info: FileInfo, hunk: Hunk, pr_details: PRDetails) -> List[Dict[str, str]]:
...
def get_ai_response(prompt: str) -> List[Dict[str, str]]:
    ...
    gemini_model = Client.GenerativeModel(os.environ.get('GEMINI_MODEL', 'gemini-2.5-pro'))
